...
對待測程序執行的測試的程度稱爲測試的覆蓋率。測試覆蓋率併不能量化——卽使最簡單的程序的動態也是難以精確測量的——但是有啟發式方法來幫助我們編寫有效的測試代碼。
...
這個標誌參數通過在測試代碼中插入生成鉤子來統計覆蓋率數據。也就是説，在運行每個測試前，它將待測代碼拷貝一份併做脩改，在每個詞法塊都會設置一個布爾標誌變量。當被脩改後的被測試代碼運行退出時，將統計日誌數據寫入c.out文件，併打印一部分執行的語句的一個總結。（如果你需要的是摘要，使用`go test -cover`。）
...
緑色的代碼塊被測試覆蓋到了，紅色的則表示沒有被覆蓋到。爲了清晰起見，我們將背景紅色文本的背景設置成了陰影效果。我們可以馬上發現unary操作的Eval方法併沒有被執行到。如果我們針對這部分未被覆蓋的代碼添加下面的測試用例，然後重新運行上面的命令，那麽我們將會看到那個紅色部分的代碼也變成緑色了：
