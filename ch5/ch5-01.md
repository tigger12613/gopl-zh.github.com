...
x和y是形參名，3和4是調用時的傳入的實參，函數返迴了一個float64類型的值。
返迴值也可以像形式參數一樣被命名。在這種情況下，每個返迴值被聲明成一個局部變量，併根據該返迴值的類型，將其初始化爲該類型的零值。
...
函數的類型被稱爲函數的籤名。如果兩個函數形式參數列表和返迴值列表中的變量類型一一對應，那麽這兩個函數被認爲有相同的類型或籤名。形參和返迴值的變量名不影響函數籤名，也不影響它們是否可以以省略參數類型的形式表示。
...
實參通過值的方式傳遞，因此函數的形參是實參的拷貝。對形參進行脩改不會影響實參。但是，如果實參包括引用類型，如指針，slice(切片)、map、function、channel等類型，實參可能會由於函數的間接引用被脩改。

你可能會偶爾遇到沒有函數體的函數聲明，這表示該函數不是以Go實現的。這樣的聲明定義了函數籤名。
