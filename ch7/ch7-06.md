...
printTracks函數將播放列表打印成一個表格。一個圖形化的展示可能會更好點，但是這個小程序使用text/tabwriter包來生成一個列整齊對齊和隔開的表格，像下面展示的這樣。註意到`*tabwriter.Writer`是滿足io.Writer接口的。它會收集每一片寫向它的數據；它的Flush方法會格式化整個表格併且將它寫向os.Stdout（標準輸出）。
